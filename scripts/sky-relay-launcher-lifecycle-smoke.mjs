...
function usage() {
  return `Usage: node scripts/sky-relay-launcher-lifecycle-smoke.mjs [options]

Runs a Launcher-owned lifecycle smoke against downloaded Sky Relay draft assets.
...
Options:
  ...
  --work-root <path>      Temporary install root. Default: tmp/sky-relay-launcher-lifecycle-smoke
  --out <path>            Evidence output path.
                          Default: ../ECHO-Release-Index/release-readiness/sky-relay-launcher-lifecycle-smoke.json
...
function parseArgs(argv) {
  ...
  const args = {
    ...
    workRoot: path.resolve(root, 'tmp', 'sky-relay-launcher-lifecycle-smoke'),
    out: path.resolve(root, '..', 'ECHO-Release-Index', 'release-readiness', 'sky-relay-launcher-lifecycle-smoke.json'),
    clean: false,
    help: false,
  }
...
async function verifyInstall(manifest, installRoot) {
  ...
  for (const file of manifest.files ?? []) {
    ...
    try {
      ...
      if (actual !== String(file.sha256).toLowerCase() || stat.size !== Number(file.size)) {
...
async function preparePreviousInstallFixture(manifest, installRoot) {
  ...
  if (!targetFile) throw new Error(`${manifest.pack}: no manifest files available for update fixture`)
  ...
  const previousManifest = {
    ...
    files: [
        ...
        : file),
      {
        ...
        required: true,
        moduleId: 'sky-relay-obsolete-smoke',
      },
    ],
  }
...
async function repairFromPackZip(manifest, zip, installRoot) {
  const targetFile = (manifest.files ?? []).find((file) => file.moduleId === 'echoskyrelayprotocol') ?? manifest.files?.[0]
  if (!targetFile) throw new Error(`${manifest.pack}: no manifest files available for repair fixture`)
  const targetPath = path.join(installRoot, targetFile.path)
  await fs.writeFile(targetPath, Buffer.from('corrupted launcher repair smoke file\n', 'utf8'))
...
async function buildCatalogEntries({ edition, manifest, manifestPath, moduleArtifactPath, moduleFile }) {
  ...
  const addonEntry = {
    ...
    trust: 'local-sky-relay-draft-smoke',
    validation: 'approved',
  }
  const packEntry = {
    id: edition.packId,
    ...
    dependencies: [{ id: 'echoskyrelayprotocol', kind: 'addon', version: '*' }],
    compatibility: [edition.packId],
    trust: 'local-sky-relay-draft-smoke',
    validation: 'approved',
  }
...
async function smokeEdition(args, edition) {
  ...
  for (const required of ['.echo/pack-manifest.json', '.echo/export-report.json', '.echo/checksums.sha256']) {
    zip.getEntry(required)
    if (!zip.getEntry(required)) throw new Error(`${edition.repoName}: missing ${required}`)
  }

  const moduleFile = (manifest.files ?? []).find((file) => file.moduleId === 'echoskyrelayprotocol')
  if (!moduleFile) throw new Error(`${edition.repoName}: manifest missing echoskyrelayprotocol file`)
  ...
  const addonLink = `echo://install/addon/echoskyrelayprotocol?pack=${edition.packId}`
