...
async function run() {
  ...
  try {
    ...
    const updateActionState = await waitForCurrentOrUpdateCardAction(cdp, SMOKE_PACK.name, args.timeoutMs, 'Sky Relay scoped current/update action')
    let updateClick = {
      clicked: false,
    ...
    if (updateActionState.acceptedAction === 'update') {
      ...
      updateClick = await waitFor('Sky Relay scoped update button', args.timeoutMs, async () => clickCardButtonContaining(cdp, SMOKE_PACK.name, 'Update'))
